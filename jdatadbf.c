...
typedef struct {
  ...
  int * size_outbuff;        // current size of target output buffer
} my_destination_mgr;
...
empty_output_buffer (j_compress_ptr cinfo)
{
  ...
  if (dest->outbuff) {
    MEMCOPY(dest->outbuff + *(dest->size_outbuff), dest->buffer, OUTPUT_BUF_SIZE);
    *(dest->size_outbuff) += OUTPUT_BUF_SIZE;
  }
  dest->pub.next_output_byte = dest->buffer;
  dest->pub.free_in_buffer = OUTPUT_BUF_SIZE;
...
term_destination (j_compress_ptr cinfo)
{
  ...
  if (datacount > 0) {
       if (dest->outbuff) {
         MEMCOPY(dest->outbuff + *(dest->size_outbuff), dest->buffer, datacount);
         *dest->size_outbuff += datacount;
       }
  }
}
...
GLOBAL(void)
jpeg_buffer_dest (j_compress_ptr cinfo, JOCTET* outbuff, int *current_size)
{
  my_dest_ptr dest;
  ...
  dest->size_outbuff = current_size;
}
